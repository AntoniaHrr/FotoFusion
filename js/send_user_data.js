(function () {
  ...
  toRegistrationBtn.addEventListener("click", () => {
    window.location.href = "./registration.html"; // redirect the user over to the registration page with an option to go back from the browser
  });
    ...
    checkLoginData(data)
      .then((responseMessage) => {
        if (responseMessage["status"] === "ERROR") {
          ...
          window.location.replace("./homepage.html"); // if the login resulted in success, then redirect the user over to his account page
        }
      })
      .catch((errorMessage) => {
